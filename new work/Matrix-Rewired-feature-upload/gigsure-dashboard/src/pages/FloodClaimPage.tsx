...
const FloodClaimPage = () => {
  const navigate = useNavigate();
  const [step, setStep] = useState<"upload" | "verify" | "submitted">(
    "upload"
  );
  const [videoFile, setVideoFile] = useState<File | null>(null);
  const [videoPreview, setVideoPreview] = useState("");
  const [verification, setVerification] = useState({
    waterLevelProof: false,
    locationProof: false,
    disruptionScope: false,
  });
  ...
  const allVerified =
    verification.waterLevelProof &&
    verification.locationProof &&
    verification.disruptionScope &&
    videoFile;
    ...
    <div className="min-h-screen bg-background pb-6">
      ...
      <div className="container py-6 max-w-2xl space-y-6">
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-cyan-500/20 to-cyan-600/5 border border-cyan-200 rounded-xl p-6 text-center"
        >
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            transition={{ delay: 0.1, type: "spring" }}
            className="h-12 w-12 mx-auto mb-3 rounded-full bg-cyan-500/10 flex items-center justify-center"
          >
          ...
          >
              ...
              {videoPreview ? (
                ...
                <div className="border-2 border-dashed border-border rounded-lg p-8 text-center hover:border-cyan-500/50 transition-colors">
                  ...
                  <label
                    ...
                    className="inline-block px-4 py-2 bg-cyan-500 text-white font-medium rounded-lg cursor-pointer hover:brightness-110 transition-all"
                  >
                    Select Video
                  </label>
                </div>
            ...
            >
              ...
              <div className="space-y-3">
                {[
                  {
                    key: "waterLevelProof",
                    label: "Water level clearly visible",
                    hint: "Show height and flow of water",
                  },
                  {
                    key: "locationProof",
                    label: "Location identification visible",
                    hint: "Street sign or building reference",
                  },
                  {
                    key: "disruptionScope",
                    label: "Flood scope and duration shown",
                    hint: "Show the affected area and timeline",
                  },
                ].map((item) => (
                  <motion.label
                    key={item.key}
            ...
            <motion.button
              ...
              className={`w-full py-3 px-4 rounded-lg font-semibold transition-all flex items-center justify-center gap-2 ${
                allVerified && !loading
                  ? "bg-cyan-500 text-white hover:brightness-110"
