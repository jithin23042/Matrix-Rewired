...
const PremiumPlanPage = () => {
  ...
  const [customPremium, setCustomPremium] = useState(65);
  const [coverageAmount, setCoverageAmount] = useState(650);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch plan details
        const planResponse = await fetch("http://localhost:5000/insurance/premium");
        if (planResponse.ok) {
          planData = await planResponse.json();
          setPlan(planData);
          setCustomPremium(planData.weeklyPremium);
          setCoverageAmount(planData.weeklyPremium * 12);
        }
        ...
        if (workerId) {
          ...
          if (statusResponse.ok) {
            ...
            if (statusData.insuranceActive && statusData.insurancePlan === 'premium') {
              setSubscribed(true);
              setCustomPremium(statusData.customPremium || planData?.weeklyPremium);
              setCoverageAmount(statusData.coverageAmount || (planData?.weeklyPremium * 12));
            }
          }
        }
      } catch (err) {
