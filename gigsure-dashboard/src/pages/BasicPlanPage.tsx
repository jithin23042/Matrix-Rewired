...
const BasicPlanPage = () => {
  ...
  const [customPremium, setCustomPremium] = useState(45);
  const [coverageAmount, setCoverageAmount] = useState(450);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch plan details
        const planResponse = await fetch("http://localhost:5000/insurance/basic");
        if (planResponse.ok) {
          planData = await planResponse.json();
          setPlan(planData);
          setCustomPremium(planData.weeklyPremium);
          setCoverageAmount(planData.weeklyPremium * 10);
        }
        ...
        if (workerId) {
          ...
          if (statusResponse.ok) {
            ...
            if (statusData.insuranceActive && statusData.insurancePlan === 'basic') {
              setSubscribed(true);
              setCustomPremium(statusData.customPremium || planData?.weeklyPremium);
              setCoverageAmount(statusData.coverageAmount || (planData?.weeklyPremium * 10));
            }
          }
        }
      } catch (err) {
